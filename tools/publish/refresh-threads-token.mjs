...
const accessToken = options.access_token ?? process.env.THREADS_ACCESS_TOKEN;
if (!accessToken) {
  console.error("Missing THREADS_ACCESS_TOKEN. Generate a fresh Threads token first.");
  process.exit(1);
}
...
try {
  ...
  printSummary("threads token refresh complete", {
    envPath,
